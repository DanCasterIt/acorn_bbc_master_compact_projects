...
## Quick start
* Ensure to program an *PROM like in the section [How to program the ROMs for this PCB design](#How-to-program-the-ROMs-for-this-PCB-design) in this file
...
The `*ROMS` command will list the ROMs `0` and `1` starting from the one at the bottom partition of the memory listed as `ROM 0`, through the one at the top partition of the system ROM listed as `ROM 1`. \
More specifically:
```
...
After programming and installing the *PROM on the cartridge, the result we will have:
```
